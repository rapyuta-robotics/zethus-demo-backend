...
def publish_marker():
    for type in types:
        ...
        for i in range(6):
            point = Point()
            point.x += i
            point.y = index + index*random.uniform(-0.4, 0.4)
            point.z = 0
...
if __name__ == "__main__":
    try:
        ...
        types = ['arrow', 'cube', 'sphere', 'cylinder', 'line_strip', 'cube_list', 'sphere_list', 'points']
        marker_publisher = []
        marker_array_publisher = []

        for type in types:
            marker_publisher.append(rospy.Publisher(type, Marker, queue_size=1, latch=True))# = rospy.Publisher(type, Marker, queue_size=10, latch=True)
            marker_array_publisher.append(rospy.Publisher(type+'_array', MarkerArray, queue_size=1, latch=True)) #= rospy.Publisher(type+'_array', MarkerArray, queue_size=10, latch=True)
